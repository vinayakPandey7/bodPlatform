...
export default function AdminCandidatesPage() {
  ...
  const handleSelectCandidate = async (candidateId: string) => {
    try {
      await api.put(`/admin/candidates/${candidateId}/status`, {
        status: "selected",
      });
      setCandidates(
  ...
  const handleUpdateStatus = async (candidateId: string, newStatus: string) => {
    try {
      ...
      setCandidates(
        candidates.map((c) =>
          c._id === candidateId ? { ...c, status: newStatus as any } : c
        )
      );
    } catch (err: any) {
                            ...
                            candidate.status !== "rejected" && (
                              <>
                                <button
                                  ...
                                  className="flex items-center justify-center px-4 py-2 bg-gradient-to-r from-emerald-600 to-green-600 text-white rounded-lg text-xs font-bold hover:from-emerald-700 hover:to-green-700 focus:outline-none focus:ring-2 focus:ring-emerald-300 transition-all duration-300 shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
                                >
                                  <svg
                                    className="w-4 h-4 mr-1"
                                    fill="none"
                                ...
                                <button
                                  ...
                                  className="flex items-center justify-center px-4 py-2 bg-gradient-to-r from-red-600 to-rose-600 text-white rounded-lg text-xs font-bold hover:from-red-700 hover:to-rose-700 focus:outline-none focus:ring-2 focus:ring-red-300 transition-all duration-300 shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
                                >
                                  <svg
                                    className="w-4 h-4 mr-1"
                                    fill="none"
