...
export default function EmployerSavedCandidatesPage() {
  ...
  const fetchSavedCandidates = async () => {
    try {
      const response = await api.get("/admin/candidates?saved=true");
      const savedCandidates =
        response.data.candidates?.filter(
          (candidate: Candidate) => candidate.isSaved
        ) || [];
  ...
  const toggleSaveCandidate = async (
    ...
    try {
      await api.patch(`/admin/candidates/${candidateId}/save`, {
        isSaved: !currentSavedStatus,
      });
      fetchSavedCandidates(); // Refresh the list
    } catch (error) {
      console.error("Error toggling candidate save status:", error);
    }
  };
              ...
              {candidates.map((candidate) => (
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <button
                      onClick={() =>
                        toggleSaveCandidate(candidate._id, candidate.isSaved)
                      }
                  ...
                  <div className="border-t pt-4">
                    <div className="flex items-center justify-between text-sm">
                      <div className="text-gray-500">
                        Submitted by: {candidate.recruitmentPartner.ownerName}
                      </div>
                  ...
                  <div className="flex space-x-2 mt-4">
                    ...
                    <button
                      ...
                      className="bg-red-600 text-white text-sm py-2 px-3 rounded-md hover:bg-red-700 transition-colors"
                    >
                      Remove
                    </button>
                  </div>
