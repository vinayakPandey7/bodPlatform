...
export default function SalesDashboard() {
  ...
  const totalClients = assignedAgents.reduce(
    (sum, agent) => sum + agent.clientsCount,
    0
  );
  const totalPending = assignedAgents.reduce(
    (sum, agent) => sum + agent.pendingClients,
    0
  );
  const totalCompleted = assignedAgents.reduce(
    (sum, agent) => sum + agent.completedClients,
    0
  );
