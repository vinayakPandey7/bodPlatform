...
const GoogleCalendarView: React.FC<GoogleCalendarViewProps> = memo(({
  ...
  const getCandidateColor = (candidateName: string, candidateEmail: string) => {
  ...
  const getCandidateBackgroundColor = (candidateName: string, candidateEmail: string) => {
    return getCandidateColor(candidateName, candidateEmail).bg;
  };

  const getCandidateBorderColor = (candidateName: string, candidateEmail: string) => {
    return getCandidateColor(candidateName, candidateEmail).border;
  };

  const getCandidateTextColor = (candidateName: string, candidateEmail: string) => {
  ...
  const getBookingsForDate = useCallback((date: Date) => {
    ...
    return bookings.filter((booking) => {
      const bookingDate = new Date(booking.slotDate).toISOString().split('T')[0];
  ...
  const handleBookingClick = useCallback((booking: BookingDetails) => {
    setSelectedBooking(booking);
    setShowBookingDetails(true);
    resetParticipantState();
    onBookingClick(booking);
  }, [onBookingClick, resetParticipantState]);

  const handleStatusUpdateClick = useCallback((booking: BookingDetails) => {
                  ...
                  <Box display="flex" flexDirection="column" gap={0.5}>
                    {dayBookings.map((booking) => (
                      <Box
                        ...
                        sx={{
                          backgroundColor: getCandidateBackgroundColor(booking.candidateName, booking.candidateEmail),
                          borderLeft: `3px solid ${getCandidateBorderColor(booking.candidateName, booking.candidateEmail)}`,
                          borderRadius: '4px',
                          padding: '4px 8px',
                          cursor: 'pointer',
                          fontSize: '11px',
                          fontWeight: 500,
                          color: getCandidateTextColor(booking.candidateName, booking.candidateEmail),
                          transition: 'all 0.2s ease',
                          '&:hover': {
                            transform: 'translateY(-1px)',
                            boxShadow: '0 2px 8px rgba(0,0,0,0.15)',
                            backgroundColor: getCandidateBorderColor(booking.candidateName, booking.candidateEmail),
                            color: 'white',
                          },
                          display: 'flex',
                          alignItems: 'center',
                      ...
                      >
                        ...
                        >
                          {`${formatTime(booking.slotStartTime)} - ${booking.candidateName}`}
                        </Box>
                      </Box>
                    ))}
                  </Box>
      ...
      <Card variant="outlined">
        <CardContent sx={{ p: 0 }}>
          ...
          <Box 
            ...
            sx={{ backgroundColor: 'grey.50', borderBottom: 1, borderColor: 'divider' }}
          >
            ...
            {days.map((date: Date, index: number) => {
              ...
              return (
                <Box
                  ...
                  alignItems="center"
                  py={2}
                  sx={{ borderRight: index < 6 ? 1 : 0, borderColor: 'divider' }}
                >
          ...
          <Box sx={{ maxHeight: 600, overflow: 'auto' }}>
            {hours.map((hour) => (
              <Box
                ...
                display="grid"
                gridTemplateColumns="80px repeat(7, 1fr)"
                sx={{ minHeight: 60, borderBottom: 1, borderColor: 'divider' }}
              >
                {/* Time label */}
                <Box
                  display="flex"
                  alignItems="center"
                  justifyContent="center"
                  sx={{ borderRight: 1, borderColor: 'divider', backgroundColor: 'grey.50' }}
                >
                  <Typography variant="caption" color="text.secondary">
                ...
                {days.map((date: Date, dayIndex: number) => {
                  const dayBookings = getBookingsForDate(date).filter(booking => {
                    const startHour = parseInt(booking.slotStartTime.split(':')[0]);
                  ...
                  <Box flex={1} p={0.5}>
                    {hourBookings.map((booking) => (
                      <Box
                        ...
                        sx={{
                          backgroundColor: getCandidateBackgroundColor(booking.candidateName, booking.candidateEmail),
                          borderLeft: `4px solid ${getCandidateBorderColor(booking.candidateName, booking.candidateEmail)}`,
                          borderRadius: '6px',
                          padding: '8px 12px',
                          cursor: 'pointer',
                          fontSize: '12px',
      ...
      >
        {selectedBooking && (
          <>
            ...
            <Box
              sx={{
                background: `linear-gradient(135deg, ${getCandidateBorderColor(selectedBooking.candidateName, selectedBooking.candidateEmail)} 0%, ${getCandidateBackgroundColor(selectedBooking.candidateName, selectedBooking.candidateEmail)} 100%)`,
                color: 'white',
                p: 3,
                position: 'relative',
              }}
                  ...
                  <Box display="flex" alignItems="center" gap={2} mb={2}>
                    <Box display="flex" alignItems="center" gap={1}>
                      ...
                      <Typography variant="body1">
                        {new Date(selectedBooking.slotDate).toLocaleDateString('en-US', {
                          weekday: 'long',
                          year: 'numeric',
                          month: 'long',
                          day: 'numeric'
                        ...
                        <Box display="flex" alignItems="center" gap={2} mb={2}>
                          <Avatar
                            sx={{
                              width: 48,
                              height: 48,
                              backgroundColor: getCandidateBorderColor(selectedBooking.candidateName, selectedBooking.candidateEmail),
                              fontSize: '1.2rem',
                              fontWeight: 600,
                            }}
                          >
                    ...
                    <Box sx={{ mb: 3 }}>
                      ...
                      <Box display="flex" flexDirection="column" gap={2}>
                        <Button
                          variant="outlined"
                          size="large"
                          onClick={() => handleStatusUpdateClick(selectedBooking)}
